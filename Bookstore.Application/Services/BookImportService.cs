...
ILogger<BookImportService> logger) : IBookImportService
{
    ...
    public async Task<int> ImportBooksAsync()
    {
        ...
        try
        {
            ...
            foreach (var importBook in booksToImport)
            {
                ...
                {
                    if (authorDict.TryGetValue(authorName.ToUpperInvariant(), out var author))
                    {
                ...
                {
                    if (genreDict.TryGetValue(genreName.ToUpperInvariant(), out var genre))
                    {
            ...
            logger.LogInformation(
                ...
                booksToAdd.Count, duration.ToString(@"hh\:mm\:ss", CultureInfo.InvariantCulture));
    ...
    private async Task<Dictionary<string, Author>> GetOrCreateAuthorsAsync(List<string> authorNames)
    {
        ...
        var existingAuthors = await db.Authors
            .Where(a => authorNames.Contains(a.Name, StringComparer.OrdinalIgnoreCase))
            .ToListAsync();

        foreach (var author in existingAuthors)
        {
            authorDict[author.Name.ToUpperInvariant()] = author;
        }

        var existingAuthorNames = existingAuthors
            .Select(a => a.Name.ToUpperInvariant())
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var newAuthorNames = authorNames
            .Where(name => !existingAuthorNames.Contains(name.ToUpperInvariant()))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToList();
    ...
    private async Task<Dictionary<string, Genre>> GetOrCreateGenresAsync(List<string> genreNames)
    {
        ...
        {
            genreDict[genre.Name.ToUpperInvariant()] = genre;
        }

        var existingGenreNames = existingGenres
            .Select(g => g.Name.ToUpperInvariant())
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var newGenreNames = genreNames
            .Where(name => !existingGenreNames.Contains(name.ToUpperInvariant()))
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToList();
        ...
        if (newGenreNames.Count > 0)
        {
            ...
            var newGenres = newGenreNames.ConvertAll(name => new Genre
            {
                Name = name
            });
